...
/* CONSTANTS */
#define c 299792458   // Propagation speed in free space [m/s]
...
double f0 = 19.51e6; // Frequency of radiation [Hz]
// uint32_t f0 = 19.51e6;        // Frequency of radiation [Hz]
double d_elem = c / (2 * f0); // Distance between radiators/elements [m]
...
{
  // return f0 + ( v_t * 1000.0 * sin( theta ) / c * f0 );
  return f0;
}
...
{
  // if (elem <= K)
  //   return 180.0 * (K - 1) + k0 * (K - elem) * d_elem * sin(theta);
  // else
  //   return -1;
  switch (elem)
  {
    ...
    case 1:
      180.0 * (K - 1) + k0 *(K - 1) * d_elem *sin(theta);
      break;
    case 2:
      180.0 * (K - 1) + k0 *(K - 1) * d_elem *sin(theta);
      break;
    case 3:
      180.0 * (K - 1) + k0 *(K - 2) * d_elem *sin(theta);
      break;
    case 4:
      180.0 * (K - 1) + k0 *(K - 2) * d_elem *sin(theta);
      break;
