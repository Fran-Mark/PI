...
Con la estructura base de la simulación ya definida, se procedió a la elaboración de \textit{testbenches} (TBs) para probar el funcionamiento de la etapa de preprocesamiento. Se realizaron las siguientes simulaciones:
\todo{tabla? o items? con el nombre de la simu y su objetivo}

\subsubsection{Tono único sin ruido}

Como primera simulación se emitió un único tono sin ruido a través del agente ADC. Esto se hizo con el fin de verificar si el \textit{pipeline} estaba funcionando en términos generales.

La frecuencia elegida para el tono a transmitir fue de $19.51\un{MHz}$, esto representa la frecuencia de una señal submuestreada. Al tratarse de un tono real, el módulo de su espectro es simétrico respecto a $f = 0$ y su correspondiente frecuencia en la banda de UHF es $- 19.51\un{MHz} + 7 \cdot 65 = 435.49\un{MHz}$. Se eligió esta frecuencia teniendo en cuenta las frecuencias de las exponenciales complejas generadas en los osciladores de banda y de canal, estas son $18.5\un{MHz}$ y $1\un{MHz}$ respectivamente. Luego, a la salida de la etapa de preprocesamiento, el tono de $19.51\un{MHz}$ se habrá trasladado a 
...
\subsubsection{Tres tonos sin ruido}
En la simulación anterior se validó el correcto funcionamiento de los \textit{mixers}, pero no se evaluaron los filtros. De la simulación anterior solo se puede decir que el tono enviado atraviesa los filtros sin atenuaciones \change{Cómo sé eso? Poner imagen a escala}, no obstante, esto no implica que la banda de paso del mismo sea la que se planteó originalmente en la subsección \ref{subsec::filtros-decimacion}.

Para evaluar la implementación de los filtros, se procedió a generar tres tonos puros, también sin ruido, los cuales serán sumados y enviados por el agente ADC. Las frecuencias elegidas para estos tonos fueron
\todo{decir freqs, una para testear cada filtro y la original}
\todo{Correr la simu de vuelta y generar los datos}

\subsubsection{Tres tonos con ruido}
Resulta relevante analizar el comportamiento del sistema al agregar RABG. Para esto, partiendo de los tres tonos generados con el agente ADC en la simulación anterior, se procedió a sumar un término de ruido Gaussiano a la señal. 
...
La integración del presente proyecto con el \textit{core} de adquisición ya existente no resulta directa ya que existen diferencias que deben ser salvadas para poder lograr la incorporación de la etapa de preprocesamiento. Estas diferencias se mencionan en la tabla \todo{ref tabla}
